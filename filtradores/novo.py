...
def filtro_novo(base, convenio,data_limite, quant_bancos, comissao_minima, margem_emprestimo_limite, selecao_lotacao, selecao_vinculos, convai, equipes, configuracoes):
    ...
    for config in configuracoes:
        ...
        if margem_seguranca:
            base.loc[mask, 'valor_liberado_emprestimo'] = (base.loc[mask, 'MG_Emprestimo_Disponivel'] * margem_seguranca * coeficiente).round(2)
            base.loc[mask, 'valor_parcela_emprestimo'] = (base.loc[mask, 'MG_Emprestimo_Disponivel'] * margem_seguranca).round(2)
        else:
    ...
    if convai > 0:
        ...
        base.loc[indices_convai, 'Campanha'] = f"{convenio}_{data_hoje}_novo_convai"
